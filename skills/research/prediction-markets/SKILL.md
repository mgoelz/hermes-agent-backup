...
**When the edge dies mid-hold (news/TT result/price move erases the original mispricing), the hold-vs-sell decision is an EV comparison, not a feelings call**: EV(hold) = P(win)·shares×1 (plus any remaining path value) vs. proceeds(sell) = shares×current quote. Hold only if EV(hold) > proceeds; otherwise sell and redeploy. Example: Roglic Vuelta position after the ITT cut only 33s of the needed ~70s — fair probability dropped to ~10%, EV(hold) ≈ 81.7 vs sell proceeds ≈ 76 → hold was EV-correct despite the drawdown. The rule keeps a losing-number position that is still EV-positive and forces the exit on one that is dead even if the drawdown feels 'too big to realize'.
...
- **Legal real-money fallback: BaFin/MiCA-licensed crypto exchanges (Kraken, Coinbase DE)**. Kraken: MiCA via Central Bank of Ireland + MiFID via CySEC, SEPA free, full REST API, paper-first viable. Coinbase Germany GmbH is the only venue with a dedicated BaFin-licensed DE entity but ~0.4–0.6% fees. Strategy port that fits: volatility-fade on BTC/ETH (same mean-reversion logic, tighter trigger — 0.8%/5min on crypto ≈ 8pp/h on Manifold) + reference-arbitrage (spot vs. CME futures vs. stablecoin discount as panic gauge). Protocol before real money: 1 week paper against live Kraken prices (bot pattern: public endpoints need no auth; `KRAKEN_LIVE=1` gates real trades), then 25€-stakes. Paper bot working: `~/manifold/kraken_bot.py`.
- **Multi-coin beats BTC-only (measured over 7-day hourly data)**: BTC averaged only 0.04%/h — a 0.8%/5min fade trigger fires about once a WEEK on BTC. Alt-coins on the same exchange are the real trading surface: DOT Ø 0.99%/h (≈25× BTC), 20 hours >2%/week; ADA 0.65%, LINK 0.60%. Build the fade as a multi-coin monitor with PER-COIN dynamic triggers (trigger ≈ 2× the coin's own Ø hourly move, TP = trigger, SL = 1.5×trigger, hold ~6h) instead of one fixed threshold, and gate every order on order-book depth (≥5k€ within 0.5% of mid) and spread (<0.3%) — DOT/EUR passed easily at our size (26k€ depth, 0.078% spread), so 25-100€ orders are invisible. Scan recipe: pull hourly OHLC per pair via the public endpoint, rank by mean |move|/h plus count of hours ≥1%/2%; check `AssetPairs` for exact pair names first (Kraken names are irregular: XXBTZEUR, XETHZEUR, DOTEUR, XXRPZEUR). Paper the multi-coin bot (`kraken_multi_bot.py`) for a week, compare hit-rate per coin, then live-enable only the coins whose mean-reversion pattern held.
- **Kraken API gotcha**: pair names are `XXBTZEUR` (BTC/EUR) / `XBTUSD` — plain `XBTZEUR` returns `EQuery:Unknown asset pair`. Enumerate via `GET /0/public/AssetPairs` instead of guessing.

### Crypto fade: backtest the BASELINE before trusting live paper results
- A winning live paper streak can be pure market luck: 27 paper trades at ~67% WR while the SAME parameters replayed over 30 days of hourly OHLC were net NEGATIVE (−0.05%/trade). Before optimizing anything, replay the current config on historical candles; if the baseline is negative, treat the live streak as noise and optimize instead of celebrating it.
- Backtest recipe (`backtest_fade.py` in ~/manifold/): Kraken OHLC `interval=60` returns max 720 candles (~30 days) — cache to disk; replicate the bot logic exactly (trigger → EMA12/26 regime filter → RSI gate → fade → TP/SL/timeout); within one candle assume SL fills BEFORE TP (conservative); disallow overlapping trades; grid-search trigger×/RSI/TP×/SL×/hold/regime and rank by avg %/trade with a minimum trade count (~30) so rare configs don't win on 3 lucky fills.
- Measured optima (30d, 10 pairs): trigger ×1.2 of the coin's volatility base; hold 6h — short timeouts were the single biggest factor (they cut winners mid-reversion); asymmetric RSI gates: fade-BUY only RSI<35 (83% WR, +0.8%/trade), fade-SELL only RSI>70 (the 65–70 bucket was negative); fade-BUY is the structurally stronger side — dips revert harder than spikes.
- Trust the structure, not the digits: top grid configs cluster (regime filter on, TP=trigger, SL=1.5×TP, long hold) — adopt the structure; exact cumulative % over a 30-day hourly window is fragile. Expect live to land well under backtest, same as Manifold.
- Size by backtest rank: cut the stake on negative-EV pairs instead of dropping them — a small stake keeps live data flowing so the verdict can be re-checked with fresh data.
- **Tax asymmetry by venue** (see references/germany-tax.md): prediction-market winnings = unsettled three-way classification; crypto exchange gains = settled § 23 law (1-year holding = tax-free). This favors the crypto-exchange path for near-term real money.
...
- **Comment lock**: comments on OTHER users' markets are blocked for **7 days after signup** (POST /v0/comment → 403 'unlocks 7 days after signup'); bets work immediately. The lock counts EXACT 7×24h from the signup timestamp, not calendar days — 'it's calendar day 7' can still 403 (bitten once: a late-day-UTC signup kept the lock active through most of calendar day 7). Schedule the criteria-clarification cron daily and let it retry until the 403 clears, instead of firing once on the calendar date.
...
- **Portfolio-level position cap (bitten hard)**: never let a single position exceed 15–20% of total portfolio, regardless of thesis conviction. Two concurrent 20% positions (Millennium-NO M$101, Vuelta-YES M$100) both went against the theses simultaneously → −40% portfolio drawdown in one week. Even a 'near-certain criteria edge' is a probability, not a certainty — the unforeseeable (news shock, external event, creator behavior) hits exactly when sizing was justified by conviction. Diversify the *edge source* (different markets, different mechanisms), not just the market names.
